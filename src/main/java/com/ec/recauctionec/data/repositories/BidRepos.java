...
public interface BidRepos extends JpaRepository<Bid, Integer> {
    ...
    @Query("select a from Bid a " +
            ...
            "and date(a.endedDate) = :date")
    List<Bid> findAllActiveAuctionByDate(@Param("date") Date date);

    @Query("select a from Bid a" +
            " where a.user.userId = :userId and (date(a.startDate)<=:date and date(a.endedDate)>=:date)")
    ...
    @Query("select a from Bid a where " +
            "date(a.createdDate)<=:currentDay " +
            "and date(a.endedDate)>=:currentDay " +
            "and a.isComplete=false " +
            "order by a.endedDate asc")
    ...
    @Query("select a from Bid a where a.user.userId=?1 order by a.createdDate desc")
